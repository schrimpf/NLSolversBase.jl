...
struct TwiceDifferentiableConstraints{F,J,H,T,TJ} <: AbstractConstraints
    c!::F # c!(storage, x) stores the value of the constraint-functions at x
    jacobian!::J # jacobian!(storage, x) stores the Jacobian of the constraint-functions
    h!::H   # h!(storage, x) stores the hessian of the constraint functions
    bounds::ConstraintBounds{T} 
    J::TJ # storage for the Jacobian         
end
...
"""
    TwiceDifferentiableConstraints(c!, jacobian!, h!, lx, ux, lc, uc;
        J=zeros(eltype(lx), length(lc), length(lx)))

Construct twice differentiable constraints.

# Arguments
- `c!` in-place constraint function `c!(storage, x)`
- `jacobian!` in-place Jacobian function `jacobian!(storage, x)`
- `h!` in-place hessian function `h!(storage, x, λ)` should add the hession of λ'constraints(x) to `storage`
- `lx` lower bounds on the x variables
